...
void GraphSolver::addmeasurement_uv(double timestamp, std::vector<uint> leftids, std::vector<Eigen::Vector2d> leftuv, const rclcpp::Logger& logger) {
  ...
  if(!systeminitalized) {
      ...
      if (ct_state == 1) {
		// https://github.com/borglab/gtsam/issues/1179
        RCLCPP_INFO(logger, "[UV] - Adding additional prior for 'ct_state == 1'");
        // Add additional prior to the first predicted state to avoid an ill-formed problem for ISAM2
        ...
        graph_new->add(gtsam::PriorFactor<gtsam::Pose3>(  X(ct_state), newstate.pose(), pose_noise));
        graph_new->add(gtsam::PriorFactor<gtsam::Vector3>(V(ct_state), newstate.v(),    v_noise));
        graph_new->add(gtsam::PriorFactor<gtsam::Bias>(   B(ct_state), newstate.b(),    b_noise));
        graph->add(gtsam::PriorFactor<gtsam::Pose3>(      X(ct_state), newstate.pose(), pose_noise));
        graph->add(gtsam::PriorFactor<gtsam::Vector3>(    V(ct_state), newstate.v(),    v_noise));
        graph->add(gtsam::PriorFactor<gtsam::Bias>(       B(ct_state), newstate.b(),    b_noise));
      }
...
//    ct_state, graph_new->size(), values_new.size(), smart_factors_to_remove_.size());
  RCLCPP_INFO(logger, "[OPT] ct_state=%zu  graph_new=%zu  values_new=%zu",
  ct_state, graph_new->size(), values_new.size());
